...
namespace MinecraftBackend.Controllers
{
    ...
    public class GameApiController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetShop(int page = 1, int pageSize = 10)
        {
            var items = await _context.ShopItems
                ...
                .Select(i => new ShopItemDto
                {
                    ...
                    ImageURL = i.ImageURL, PriceAmount = i.PriceAmount, PriceCurrency = i.PriceCurrency,
                    Rarity = i.Rarity, Type = i.ItemType, TargetItemID = i.TargetItemID
                }).ToListAsync();
        ...
        [AllowAnonymous] [HttpPost("sim/craft")] public async Task<IActionResult> SimCraft(string charId, string recipeId) {
            var profile = await _context.PlayerProfiles.FirstOrDefaultAsync(p => p.CharacterID == charId);
            if (profile == null) return NotFound("Character not found");
        ...
        [AllowAnonymous] [HttpGet("affordable/{charId}")] public async Task<IActionResult> CheckAffordable(string charId) {
            var profile = await _context.PlayerProfiles.FirstOrDefaultAsync(p => p.CharacterID == charId);
            if (profile == null) return NotFound();
