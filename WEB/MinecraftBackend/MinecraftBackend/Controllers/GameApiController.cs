...
namespace MinecraftBackend.Controllers
{
    ...
    public class GameApiController : ControllerBase
    {
        ...
        {
            var recipes = await _context.Recipes.Select(r => new { r.RecipeId, r.ResultItemName, r.ResultItemImage, r.CraftingTime, IngredientsStr = r.Ingredients }).ToListAsync();
            return Ok(recipes);
        }
        ...
        public async Task<IActionResult> CraftItem(string recipeId)
        {
            ...
            foreach (var part in ingredients)
            {
                ...
                if (split.Length == 2 && int.TryParse(split[1], out int qty)) requiredItems[split[0]] = qty;
            }
            ...
            foreach (var req in requiredItems)
            {
                ...
                if (itemInInv == null || itemInInv.Quantity < req.Value) return BadRequest(new { message = $"Thiếu nguyên liệu: {req.Key}" });
            }
        ...
        public async Task<IActionResult> HuntMonster()
        {
            ...
            p.Exp += 5;
            bool lvUp = false;
            if (p.Exp >= 100 * p.Level) { p.Level++; p.Exp = 0; lvUp = true; }
            await _context.SaveChangesAsync();
