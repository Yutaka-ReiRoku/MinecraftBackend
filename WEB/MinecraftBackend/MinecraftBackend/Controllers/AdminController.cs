    ...
    public class AdminController : Controller
    {
        ...
        )
        {
            ...
            query = sortOrder switch
            {
                ...
                "Type" => query.OrderBy(s => s.ItemType), "type_desc" => query.OrderByDescending(s => s.ItemType),
                "Rarity" => query.OrderBy(s => s.Rarity), "rarity_desc" => query.OrderByDescending(s => s.Rarity),
                "Price" => query.OrderBy(s => s.PriceAmount), "price_desc" => query.OrderByDescending(s => s.PriceAmount),
        ...
        public async Task<IActionResult> CreateItem(ShopItem item, IFormFile? imageFile)
        {
            ...
            if (imageFile != null) item.ImageURL = await SaveImage(imageFile, item.ItemType); else item.ImageURL = "/images/others/default.png";
            ...
            {
                if (await _context.ShopItems.AnyAsync(i => i.ProductID == item.ProductID)) { ModelState.AddModelError("ProductID", "ID này đã tồn tại!"); return View(item); }
                _context.ShopItems.Add(item); await _context.SaveChangesAsync();
        ...
        public async Task<IActionResult> EditItem(ShopItem item, IFormFile? imageFile)
        {
            ...
            if (imageFile != null) item.ImageURL = await SaveImage(imageFile, item.ItemType); else item.ImageURL = existing.ImageURL;
            if (string.IsNullOrEmpty(item.TargetItemID)) item.TargetItemID = item.ProductID;
            if (ModelState.IsValid) { _context.ShopItems.Update(item); await _context.SaveChangesAsync(); return RedirectToAction("Items", new { tab = item.IsShow ? "shop" : "data" }); }
            return View(item);
        }
        ...
        public async Task<IActionResult> DeleteItem(string id)
        {
            ...
            if (item != null) { returnTab = item.IsShow ? "shop" : "data"; _context.ShopItems.Remove(item); await _context.SaveChangesAsync(); }
        ...
        {
             if (string.IsNullOrEmpty(username) || string.IsNullOrEmpty(email) || string.IsNullOrEmpty(password)) { ModelState.AddModelError("", "Vui lòng nhập đủ thông tin!"); return View(); }
            ...
            var newUser = new User { Id = Guid.NewGuid().ToString(), Username = username, Email = email, PasswordHash = BCrypt.Net.BCrypt.HashPassword(password), Role = "User", Status = "Active", CreatedAt = DateTime.Now };
            _context.Users.Add(newUser);
            var newProfile = new PlayerProfile { CharacterID = Guid.NewGuid().ToString(), UserId = newUser.Id, DisplayName = username, Level = 1, Exp = 0, Gold = 2000, Gem = 50, Health = 100, MaxHealth = 100, Hunger = 100, AvatarUrl = "/images/avatars/steve.png", GameMode = "Survival" };
        ...
        public async Task<IActionResult> EditUser(PlayerProfile profile, IFormFile? avatarFile)
        {
            ...
            existing.DisplayName = profile.DisplayName; existing.Level = profile.Level; existing.Gold = profile.Gold; existing.Gem = profile.Gem; existing.GameMode = profile.GameMode;
            await _context.SaveChangesAsync(); return RedirectToAction("UserDetails", new { id = existing.UserId });
        }

        public async Task<IActionResult> ToggleUserStatus(string id)
        {
            var user = await _context.Users.FindAsync(id);
            if (user != null) { user.Status = (user.Status == "Active") ? "Banned" : "Active"; await _context.SaveChangesAsync(); }
            return RedirectToAction("Users");
        }
        ...
        {
            if (string.IsNullOrEmpty(newPassword) || newPassword.Length < 6) { TempData["Error"] = "Mật khẩu mới phải dài hơn 6 ký tự!"; return RedirectToAction("Users"); }
            var user = await _context.Users.FindAsync(userId);
            if (user != null) { user.PasswordHash = BCrypt.Net.BCrypt.HashPassword(newPassword); await _context.SaveChangesAsync(); TempData["Message"] = $"Đã đổi mật khẩu cho user: {user.Username}"; }
        ...
        public async Task<IActionResult> GiveGift(string userId, string type, int amount, string? itemId)
        {
            ...
            if (profile != null)
            {
                ...
                else if (type == "Item" && !string.IsNullOrEmpty(itemId)) { _context.Inventories.Add(new GameInventory { InventoryId = Guid.NewGuid().ToString(), UserId = profile.UserId, ItemID = itemId, Quantity = amount, AcquiredDate = DateTime.Now }); }
        ...
        // --- SYSTEM LOGS ---
        public async Task<IActionResult> Logs(string search = "", string userId = "", string type = "", string startDate = "", string endDate = "", string currency = "", string sortOrder = "", int page = 1)
        {
            int pageSize = 15;
            ...
            ViewBag.CurrentSort = sortOrder; ViewBag.CurrentSearch = search; ViewBag.CurrentUserId = userId; ViewBag.CurrentType = type; ViewBag.CurrentStartDate = startDate; ViewBag.CurrentEndDate = endDate; ViewBag.CurrentCurrency = currency;
            ViewBag.Types = new List<string> { "LOGIN", "REGISTER", "TRANSACTION", "GIFT", "CRAFT", "BUY", "BUILD", "SELL" };
            ...
            if (!string.IsNullOrEmpty(startDate) && DateTime.TryParse(startDate, out DateTime sDt)) query = query.Where(l => l.CreatedAt >= sDt.Date);
            if (!string.IsNullOrEmpty(endDate) && DateTime.TryParse(endDate, out DateTime eDt)) query = query.Where(l => l.CreatedAt < eDt.Date.AddDays(1)); 

            query = sortOrder switch { "date_asc" => query.OrderBy(l => l.CreatedAt), "User" => query.OrderBy(l => l.UserId), "user_desc" => query.OrderByDescending(l => l.UserId), "Action" => query.OrderBy(l => l.ActionType), "action_desc" => query.OrderByDescending(l => l.ActionType), "Amount" => query.OrderBy(l => l.Amount), "amount_desc" => query.OrderByDescending(l => l.Amount), _ => query.OrderByDescending(l => l.CreatedAt) };
